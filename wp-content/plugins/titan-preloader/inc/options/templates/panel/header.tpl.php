...
// image url
$logo = TITAN_PLUGIN_URL . 'inc/assets/images/branding/logo-admin.png';
...
<div id="redux-header">
    <?php if (!empty($this->parent->args['display_name'])) { ?>
        <div class="display_header">
            ...
            <img src="<?php echo esc_url($logo); ?>" alt="Titan Preloader & Page Transition WordPress Plugin" />

            <?php if (isset($this->parent->args['dev_mode']) && $this->parent->args['dev_mode']) { ?>
                <div class="redux-dev-mode-notice-container redux-dev-qtip" qtip-title="<?php echo esc_attr($tip_title); ?>"
                    qtip-content="<?php echo esc_attr($tip_msg); ?>">
                    <span class="redux-dev-mode-notice">
                        <?php esc_html_e('Developer Mode Enabled', 'redux-framework'); ?>
                    </span>
                </div>
                <?php } ?>
            <?php if (!empty($this->parent->args['display_version'])) { ?>
                <span class="version-number">
                    <?php echo wp_kses_post($this->parent->args['display_version']); ?>
                </span>
                <?php } ?>
        </div>
        <?php } ?>

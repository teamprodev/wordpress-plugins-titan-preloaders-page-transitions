...
(function ($) {
    ...
    $.redux.ajax_save = function (button) {
        ...
        var $notification_bar = $(
            document.getElementById('redux_notification_bar')
        );
        ...
        $.ajax({
            ...
            error: function (response) {
                ...
                const event = new CustomEvent('redux-save', {
                    detail: { class: '.redux-notification-error' },
                });
            ...
            success: function (response) {
                ...
                const event = new CustomEvent('redux-save', {
                    detail: { class: '.redux-notification-success' },
                });
                window.dispatchEvent(event);
                ...
                if (response.action && 'reload' === response.action) {
                    ...
                    $('.wrap h2:first')
                        .parent()
                        .append(
                            '<div class="error redux_ajax_save_error" style="display:none;"><p>' +
                                response.status +
                                '</p></div>'
                        );
...
(function ($) {
    ...
    $.redux.expandOptions = function (parent) {
        ...
        var id =
            $('.redux-group-menu .active a').data('rel') + '_section_group';
        ...
        if (trigger.hasClass('expanded')) {
            ...
            parent
                .find('.redux-main')
                .stop()
                .animate({ 'margin-left': width }, 500, function () {
            ...
            parent
                .find('.redux-sidebar')
                .stop()
                .animate({ 'margin-left': -width - 113 }, 500);

            parent
                .find('.redux-main')
                .stop()
                .animate({ 'margin-left': '-1px' }, 500);
...
(function ($) {
    ...
    $.redux.initEvents = function (el) {
        ...
        if (
            ...
            'string' === typeof curPage &&
            true === redux.optName.args.dev_mode
        ) {
                ...
                messageDialog = $('#redux-dialog-message').dialog({
                    classes: {
                        ...
                        'ui-dialog-buttonpane':
                            'redux-message-dialog-buttonpane',
                ...
                dialog = $('#redux-dialog-confirm').dialog({
                    ...
                    buttons: {
                        Submit: function () {
                            ...
                            var content = $(
                                '.redux-message-dialog-content .redux-message-p'
                            );
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    nonce: $('#redux-dialog-confirm').data(
                                        'nonce'
                                    ),
                                },
                                beforeSend: function () {
                                    buttonPane.css({ display: 'none' });
                                    $('#redux-dialog-message .spinner').css({
                                        visibility: 'visible',
                                    });
                                ...
                                success: function (response) {
                                    ...
                                    if (
                                        ...
                                        content.html(
                                            'Your support data has been transmitted.  The reference number for this transmission is: <strong>' +
                                                response.data +
                                                '</strong>'
                                        );
                                    } else {
        ...
        el.find('.redux-presets-bar').on('click', function () {
        ...
        el.find(
            '#toplevel_page_' +
                redux.optName.args.slug +
                ' .wp-submenu a, #wp-admin-bar-' +
                redux.optName.args.slug +
                ' a.ab-item'
        ).on('click', function (e) {
            var url;
            ...
            if (
                (el
                    .find('#toplevel_page_' + redux.optName.args.slug)
                    .hasClass('wp-menu-open') ||
                    $(this).hasClass('ab-item')) &&
                !$(this)
                    .parents('ul.ab-submenu:first')
                    .hasClass('ab-sub-secondary') &&
                $(this)
                    .attr('href')
                    .toLowerCase()
                    .indexOf(redux.optName.args.slug + '&tab=') >= 0
            ) {
        ...
        el.find('.redux-action_bar input, #redux-import-action input').on(
            ...
            function (e) {
                if (
                    $(this).attr('name') ===
                    redux.optName.args.opt_name + '[defaults]'
                ) {
                    ...
                    $(this).attr('name') ===
                    redux.optName.args.opt_name + '[defaults-section]'
                ) {
        ...
        $('.expand_options').on('click', function (e) {
            var tab;
            ...
            if ($(container).hasClass('fully-expanded')) {
                ...
                el.find('#' + tab + '_section_group').fadeIn(200, function () {
        ...
        $(document.body).on(
            ...
            function () {
                if (
                    $('.redux-container-typography select').hasClass(
                        'ignore-change'
                    )
                ) {
                    return;
                ...
                if (
                    !$(this).hasClass('noUpdate') &&
                    !$(this).hasClass('no-update')
                ) {
                    redux_change($(this));
                }
            }
        ...
        el.find('#redux-sticky-padder').css({ height: stickyHeight });
...
(function ($) {
    ...
    $.redux.initFields = function () {
        $('.redux-group-tab:visible')
            .find('.redux-field-init:visible')
            .each(function () {
                var tr;
                var th;
                ...
                if (
                    'undefined' !== typeof redux.field_objects &&
                    redux.field_objects[type] &&
                    redux.field_objects[type]
                ) {
                ...
                if (
                    'undefined' !== typeof redux.field_objects.pro &&
                    !$.isEmptyObject(redux.field_objects.pro[type]) &&
                    redux.field_objects.pro[type]
                ) {
...
(function ($) {
    ...
    $.redux.flyoutSubmenu = function () {
        ...
        $('.redux-group-tab-link-li a').on('click', function () {
            if (true === redux.optName.args.flyout_submenus) {
                $('.redux-group-tab-link-li').removeClass(
                    'redux-section-hover'
                );
            }
        });
    ...
    $.redux.disableFields = function () {
        $('label[for="redux_disable_field"]').each(function () {
            $(this)
                .parents('tr')
                .find('fieldset:first')
    ...
    $.redux.hideFields = function () {
        $('label[for="redux_hide_field"]').each(function () {
    ...
    $.redux.getSelector = function (selector, fieldType) {
        if (!selector) {
            ...
            if (redux.customizer) {
                selector = $(document)
                    .find('.control-section-redux.open')
                    .find(selector);
            } else {
                selector = $(document)
        ...
        if (redux.optName.sanitize && redux.optName.sanitize.sanitize) {
            $.each(
                ...
                function (sectionID, sectionArray) {
                    sectionID = null;
                    $.each(sectionArray.sanitize, function (key, value) {
    ...
    $.redux.notices = function () {
            ...
            $.each(
                ...
                function (sectionID, sectionArray) {
                    ...
                    $.each(sectionArray.errors, function (key, value) {
                        var fieldset = $(
                            '#' + redux.optName.args.opt_name + '-' + value.id
                        );

                        fieldset.addClass('redux-field-error');
                        if (
                            0 ===
                            fieldset.parent().find('.redux-th-error').length
                        ) {
                            fieldset.append(
                                '<div class="redux-th-error">' +
                                    value.msg +
                                    '</div>'
                            );
                        } else {
                ...
                if (totalErrors > 0) {
                    ...
                    container.find('.redux-group-tab').each(function () {
                        ...
                        if (total > 0) {
                            ...
                            container
                                .find(
                                    '.redux-group-tab-link-a[data-key="' +
                                        sectionID +
                                        '"]'
                                )
                                .prepend(
                                    '<span class="redux-menu-error">' +
                                        total +
                                        '</span>'
                                );
                            container
                            ...
                            subParent = container
                                ...
                                .parents('.hasSubSections:first');

                            if (subParent) {
                                subParent
                                    .find('.redux-group-tab-link-a:first')
                                    .addClass('hasError');
                            }
                        }
                    });
        ...
        if (redux.optName.warnings && redux.optName.warnings.warnings) {
            $.each(
                ...
                function (sectionID, sectionArray) {
                    ...
                    $.each(sectionArray.warnings, function (key, value) {
                        ...
                        if (
                            0 ===
                            fieldset.parent().find('.redux-th-warning').length
                        ) {
                            fieldset.append(
                                '<div class="redux-th-warning">' +
                                    value.msg +
                                    '</div>'
                            );
                        } else {
            ...
            $('.redux-container').each(function () {
                ...
                if (totalWarnings > 0) {
                    container
                        .find('.redux-field-warnings span')
                        .text(totalWarnings);
...
function redux_change(variable) {
    ...
    (function ($) {
        ...
        th = rContainer
            .find('.redux-group-tab-link-a[data-key="' + id + '"]')
            .parents('.redux-group-tab-link-li:first');
        li = $('#' + parentID + '_li');
        subParent = li.parents('.hasSubSections:first');

        if (
            $(variable)
                .parents('fieldset.redux-field:first')
                .hasClass('redux-field-error')
        ) {
            $(variable)
                .parents('fieldset.redux-field:first')
                .removeClass('redux-field-error');
            $(variable).parents().find('.redux-th-error').slideUp();

            errorCount =
                parseInt(
                    rContainer.find('.redux-field-errors span').text(),
                    0
                ) - 1;

            if (errorCount <= 0) {
                $('#' + parentID + '_li .redux-menu-error')
                    .fadeOut('fast')
                    .remove();
                $('#' + parentID + '_li .redux-group-tab-link-a').removeClass(
                    'hasError'
                );
                li.parents('.inside:first')
                    .find('.redux-field-errors')
                    .slideUp();
                $(variable)
                    .parents('.redux-container:first')
                    .find('.redux-field-errors')
                    .slideUp();
                $('#redux_metaboxes_errors').slideUp();
            } else {
                errorsLeft =
                    parseInt(th.find('.redux-menu-error:first').text(), 0) - 1;
        ...
        if (
            $(variable)
                .parents('fieldset.redux-field:first')
                .hasClass('redux-field-warning')
        ) {
            $(variable)
                .parents('fieldset.redux-field:first')
                .removeClass('redux-field-warning');
            $(variable).parent().find('.redux-th-warning').slideUp();

            warningCount =
                parseInt(
                    rContainer.find('.redux-field-warnings span').text(),
                    0
                ) - 1;

            if (warningCount <= 0) {
                $('#' + parentID + '_li .redux-menu-warning')
                    .fadeOut('fast')
                    .remove();
                $('#' + parentID + '_li .redux-group-tab-link-a').removeClass(
                    'hasWarning'
                ...
                warningsLeft =
                    parseInt(th.find('.redux-menu-warning:first').text(), 0) -
                    1;
...
(function ($) {
    ...
    $.redux.checkRequired = function (el) {
        ...
        el.find('.redux-container td > fieldset:empty,td > div:empty')
            .parent()
            .parent()
            .hide();
    };
    ...
    $.redux.required = function () {
        ...
        $.each(redux.opt_names, function (x) {
            $.each(
                ...
                function (i, v) {
                    ...
                    if ('hide' === v) {
                        ...
                        if (fieldset.hasClass('redux-container-raw')) {
                            rawTable = fieldset
                                .parents()
                                .find('table#' + redux.opt_names[x] + '-' + i);
    ...
    $.redux.getContainerValue = function (id) {
        var value = $(
            '#' + redux.optName.args.opt_name + '-' + id
        ).serializeForm();

        if (
            null !== value &&
            'object' === typeof value &&
            value.hasOwnProperty(redux.optName.args.opt_name)
        ) {
            value = value[redux.optName.args.opt_name][id];
        }

        if (
            $('#' + redux.optName.args.opt_name + '-' + id).hasClass(
                'redux-container-media'
            )
        ) {
            value = value.url;
        }
    ...
    $.redux.check_dependencies = function (variable) {
        ...
        if (!container.parents('tr:first').length) {
            isHidden = container
                .parents('.customize-control:first')
                .hasClass('hide');
        }

        $.each(redux.optName.required[id], function (child) {
            var div;
            var rawTable;
            var tr;
            ...
            var show = false;
            var childFieldset = $(
                '#' + redux.optName.args.opt_name + '-' + child
            );
            ...
            if (true === show) {
                ...
                if (childFieldset.hasClass('redux-container-section')) {
                    ...
                    if (
                        div.hasClass('redux-section-indent-start') &&
                        div.hasClass('hide')
                    ) {
                        $('#section-table-' + child)
                            .fadeIn(300)
                ...
                if (childFieldset.hasClass('redux-container-raw')) {
                    rawTable = childFieldset
                        .parents()
                        .find(
                            'table#' + redux.optName.args.opt_name + '-' + child
                        );
                ...
                if (
                    childFieldset.hasClass('redux-container-section') ||
                    childFieldset.hasClass('redux-container-info')
                ) {
                    tr.css({ display: 'none' });
                }
            } else if (false === show) {
            ...
            current
                .find('select, radio, input[type=checkbox]')
                .trigger('change');
        });
    };
    ...
    $.redux.required_recursive_hide = function (id) {
        ...
        toFade = $('#' + redux.optName.args.opt_name + '-' + id).parents(
            'tr:first'
        );
        if (0 === toFade) {
            toFade = $('#' + redux.optName.args.opt_name + '-' + id).parents(
        ...
        toFade.fadeOut(50, function () {
            ...
            if (
                ...
                rawTable = $('#' + redux.optName.args.opt_name + '-' + id)
                    .parents()
                    .find('table#' + redux.optName.args.opt_name + '-' + id);
    ...
    $.redux.check_parents_dependencies = function (id) {
        ...
        if (redux.optName.required_child.hasOwnProperty(id)) {
            $.each(redux.optName.required_child[id], function (i, parentData) {
                var parentValue;
                ...
                if (
                    ...
                    if (false !== show) {
                        parentValue = $.redux.getContainerValue(
                            parentData.parent
                        );

                        show = $.redux.check_dependencies_visibility(
                            parentValue,
                            parentData
                        );
                    }
...
(function ($) {
    ...
    $.redux.stickyInfo = function () {
        ...
        if (
            !$('#info_bar').isOnScreen() &&
            !$('#redux-footer-sticky').isOnScreen()
        ) {
            $('#redux-footer').css({
                position: 'fixed',
                bottom: '0',
...
(function ($) {
    ...
    $.redux.tabCheck = function () {
        ...
        $('.redux-group-tab-link-a').on('click', function () {
            ...
            if (
                link.parent().hasClass('empty_section') &&
                link.parent().hasClass('hasSubSections')
            ) {
                elements = $(this)
                    .closest('ul')
                    .find('.redux-group-tab-link-a');
            ...
            oldid = el
                .find(
                    '.redux-group-tab-link-li.active:first .redux-group-tab-link-a'
                )
                .data('rel');
            ...
            if (
                el
                    .find('#' + relid + '_section_group_li')
                    .parents('.redux-group-tab-link-li').length
            ) {
                parentID = el
                    .find('#' + relid + '_section_group_li')
                    .parents('.redux-group-tab-link-li')
                    .attr('id')
                    .split('_');
                parentID = parentID[0];
            }
            ...
            el.find(
                '#toplevel_page_' + redux.optName.args.slug + ' .wp-submenu a'
            ).each(function () {
            ...
            if (
                el
                    .find('#' + oldid + '_section_group_li')
                    .find('#' + oldid + '_section_group_li').length
            ) {
                el.find('#' + oldid + '_section_group_li').addClass(
                    'activeChild'
                );
                el.find('#' + relid + '_section_group_li')
                    .addClass('active')
                    .removeClass('activeChild');
            } else if (
                el
                    .find('#' + relid + '_section_group_li')
                    .parents('#' + oldid + '_section_group_li').length ||
                el
                    .find('#' + oldid + '_section_group_li')
                    .parents('ul.subsection')
                    .find('#' + relid + '_section_group_li').length
            ) {
                if (
                    el
                        .find('#' + relid + '_section_group_li')
                    ...
                    el.find('#' + oldid + '_section_group_li')
                        .addClass('activeChild')
                        .removeClass('active');
                } else {
                    el.find('#' + relid + '_section_group_li').addClass(
                        'active'
                    );
                    el.find('#' + oldid + '_section_group_li').removeClass(
                        'active'
                    );
                }
                el.find('#' + relid + '_section_group_li')
                ...
                if (
                    ...
                    if (newParent.length > 0) {
                        el.find('#' + relid + '_section_group_li').removeClass(
                            'active'
                        );
                        relid = newParent
                            .find('.redux-group-tab-link-a:first')
                    ...
                    el
                        .find('#' + oldid + '_section_group_li')
                        .parents('ul.subsection').length
                ) {
                    if (
                        !el
                            .find('#' + oldid + '_section_group_li')
                            .parents('#' + relid + '_section_group_li').length
                    ) {
                        el.find('#' + oldid + '_section_group_li')
                            .parents('ul.subsection')
                            .slideUp('fast', function () {
                                el.find(
                                    '#' + oldid + '_section_group_li'
                                ).removeClass('active');
                                el.find('#' + oldid + '_section_group_li')
                                    .parents('.redux-group-tab-link-li')
                                    .removeClass('active')
            ...
            el.find('#' + relid + '_section_group').fadeIn(200, function () {
                if (0 !== el.find('#redux-footer').length) {
                    $.redux.stickyInfo(); // Race condition fix.
                }
            ...
            $('#toplevel_page_' + redux.optName.args.slug)
                .find('.current')
                .removeClass('current');
        });

        if (undefined !== redux.optName.last_tab) {
            $('#' + redux.optName.last_tab + '_section_group_li_a').trigger(
                'click'
            );
        ...
        tab = decodeURI(
            (new RegExp('tab=(.+?)(&|$)').exec(location.search) || [''])[1]
        );
        ...
        $('.redux-container').each(function () {
            ...
            if (!$(this).parents('.postbox-container:first').length) {
                ...
                sTab = $(this).find(
                    '#' + $.cookie(cookieName) + '_section_group_li_a'
                );
            } else {
                opt_name = $.redux.getOptName(this);
                ...
                cookieName =
                    'redux_metabox_' + boxIndex + '_current_tab_' + opt_name;
            ...
            if (
                null === $.cookie(cookieName) ||
                'undefined' === typeof $.cookie(cookieName) ||
                0 === sTab.length
            ) {
